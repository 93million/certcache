...
From your DNS management control panel, perfrom the following steps:
  ...
  * For each domain you want to validate, add a `CNAME` record for the host `_acme-challenge.<domain-to-validate>` that points to `<domain-to-validate>.acme.<certcache-domain>`
...
You can test using `dig` on the command line. Running `dig _acme-challenge.<domain-to-validate> CNAME +short` should output `<domain-to-validate>.acme.<certcache-domain>.`. Eg:
...
You can test using `curl` on the command line. Running `curl -v http://<domain-to-validate>/.well-known/acme-challenge/foo` should output a `Location:` header showing redirection. Eg:
