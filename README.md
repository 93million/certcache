...
Now the challenges are configured you can start CertCache client. Run: `docker-compose up -d` on the CertCache client instance. Certificates will be placed in `./certcache/certs`. Map a volume to this path in containers that require access to the certificates.
